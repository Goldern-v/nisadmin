...
export class FormField extends React.Component<Props, State> {
  ...
  public render () {
    const {
      name, label, labelWidth = 120, gutter = 20, tip = true, inline,
      suffix,required, children, value, error, onChange
    } = this.props
    ...
    if (React.isValidElement(children) && name) {
      ...
      const props = type === Checkbox
        ? { ...children.props, checked: value, onChange }
...
export default function FormFieldWrapper (props: OwnProps) {
  const { name, labelWidth, ...rest } = props
      ...
      {(context) => (
        name ? (
          <FormField
            ...
            onChange={(...args) => context.onFieldChange(name, args)}
          />
        ) : (
