...
export default function GroupsAduitModal(props: Props) {
  ...
  const onOk = () => {
    ...
    let obj = {}
    if(appStore.queryObj.fileId){
      obj = {
        saveAudites: [{
          id: appStore.queryObj.fileId,
          fileId: appStore.queryObj.id,
        }],
      ...
      obj = {
        saveAudites: (selectedRows || []).map((item: any) => ({
          id: item.auditedManageDto.id,
          fileId: item.auditedManageDto.fileId,
        })),
        flagAudited: agree === 'agree' ? true : agree === 'disagree' ? false : '',
        detail: opinion
      }
    }
    modalService.auditeListJM(obj).then((res) => {
