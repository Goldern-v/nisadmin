...
export default observer(function SelectPeopleModal(props: Props) {
  let { visible, onCancel, onOkCallBack, onClose, evalType, title, showReset } = props
  ...
  useLayoutEffect(() => {
    if (visible) {
      ...
      setSearchWord('')
      // 展开
      if (evalType && evalTypeEnums[evalType]) {
        selectPeopleViewModel.pushStep(evalTypeEnums[evalType])
      } 
    }
  }, [visible])
