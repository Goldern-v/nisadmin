...
export default observer(function EventReportDetailView(props: Props) {
	...
	useEffect(() => {
		...
		setSpinning(true)
		apiSpecialNurse.getReportById({ masterId: propsData.id }).then((res: any) => {
			dataWholeAysi.master = res.data.master || {}
			dataWholeAysi.rowList = res.data.rowList || []
			dataWholeAysi.evaluationList = res.data.evaluationList || []
	...
	const turnToDel = () => {
		let params={
			masterId:dataWholeAysi.master.id
		}
	  globalModal.confirm('删除确认', '你确定要删除该报告吗？').then((res) => {
	    apiSpecialNurse.delReportById(params).then((res) => {
	...
	const onSave = async () => {
		...
		// 本来已有报告
		if(dataWholeAysi.evaluationList.length>0){
			params.evaluationList = dataWholeAysi.evaluationList
		...
		// return false
		setSpinning(true)
		apiSpecialNurse.saveNewReport(params).then(res=>{
			if(res.code=='200'){
	...
	return (
		<Wrapper>
			<HeadCon>
				<BaseBreadcrumb data={[{ name: '专科护理质量分析', link: '/goodOrBadRouter/wholeAysi' }, { name: '报告详情', link: '' }]} />
				{/* {propsData.reportType == '0' && <div className='title'>{propsData.belongsYear}年{Number(propsData.belongsCycle)}月专科护理质量汇总报告</div>} */}
				<div className='title'>{dataWholeAysi.currentCycleMessage}专科护理质量汇总报告</div>
				<div className='aside'>
					<span>
