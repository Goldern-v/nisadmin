...
export default observer(function 三级问题原因措施汇总() {
  ...
  const columns: ColumnProps<any>[] = [
    {
      ...
      render: (text: string, record: any) => {
        return (
          <SampleInput
            text={text}
            input={(e: any) => saveTableData(e, "checkDate", record)}
            save={(e: string) => saveMain(e, "checkDate", record)}
          />
        );
      },
    },
      ...
      render: (text: string, record: any) => {
        return (
          <SampleInput
            isMulti={true}
            text={text}
            input={(e: any) => saveTableData(e, "checkAuthor", record)}
            save={(e: string) => saveMain(e, "checkAuthor", record)}
          />
        );
      },
    },
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <EditList
          arr={text}
          editList={["content"]}
          input={(e: any) => saveTableData(e, "problem", record)}
          save={() => saveContent("problem", text)}
        />
      ),
    },
    {
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <EditList
          arr={text}
          editList={["content", "evaluateDate", "author"]}
          input={(e: any) => saveTableData(e, "causeAnalysis", record)}
          save={() => saveContent("causeAnalysis", text)}
        />
      ),
    },
    {
      ...
      render: (text: any, record: any) => (
        <EditList
          arr={text}
          editList={["content", "evaluateDate", "author"]}
          input={(e: any) => saveTableData(e, "rectificationMeasures", record)}
          save={() => saveContent("rectificationMeasures", text)}
        />
      ),
    },
    {
      title: "护长评价日期",
      dataIndex: "chiefNurseDate",
      align: "left",
      width: 130,
      render: (text: any, record: any) => (
        <EditList
          arr={text}
          editList={["evaluateDate"]}
          input={(e: any) => saveTableData(e, "chiefNurseDate", record)}
          save={() => saveContent("chiefNurseDate", text)}
        />
      ),
    ...
    {
      title: "护长评价",
      dataIndex: "chiefNurseEvaluation",
      align: "left",
      width: 200,
      render: (text: any, record: any) => (
        <EditList
          arr={text}
          editList={["content"]}
          input={(e: any) => saveTableData(e, "chiefNurseEvaluation", record)}
          save={() => saveContent("chiefNurseEvaluation", text)}
        />
      ),
    },
    {
      title: "护长评价者",
      dataIndex: "chiefNurseAuthor",
      align: "left",
      width: 80,
      render: (text: any, record: any) => (
        <EditList
          arr={text}
          editList={["author"]}
          input={(e: any) => saveTableData(e, "chiefNurseAuthor", record)}
          save={() => saveContent("chiefNurseAuthor", text)}
        />
      ),
    },
  ];
  ...
  const getTableData = () => {
    setLoading(true);
    qcFormFqfybjyService.problemCauseMeasureSummary(query).then(
      (res) => {
        let data = res.data;
  ...
  const handleExport = () =>
    qcFormFqfybjyService.problemCauseMeasureSummaryExport(query);
  ...
  return (
    <Wrapper>
      <PageHeader>
        <PageTitle>
          {numToChinese(Number(queryObj.qclevel || "1")) + "级"}
          质控问题原因措施汇总
        </PageTitle>
        <Place />
        <span>表单：</span>
