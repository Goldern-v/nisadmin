...
class QualityAnalysisReportViewModal {
  ...
  async initData() {
    ...
    this.getSectionData(`查房内容`).text = this.allData.report!.checkDeptDesc || ''
    this.getSectionData(`检查形式`).text = this.allData.report!.checkWayDesc || ''
    // this.getSectionData(`上月质量问题`).list = this.allData!.lastImproveItemList || []

    let bzkfqkText = ''
    let target = (this.allData!.typeCompareList || []).find((item: any) => item.itemType === '总扣分' || item.itemTypeName === '总扣分')
    if (target) bzkfqkText = (Number(parseInt((target.compareScore * 100).toString())) / 100).toString()
    this.getSectionData(`本周检查扣分情况`).text = bzkfqkText
