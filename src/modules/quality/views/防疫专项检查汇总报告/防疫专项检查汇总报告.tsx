...
export default observer(function QualityAnalysisReportView() {
  ...
  let report: Report = qualityAnalysisReportViewModal.getDataInAllData(
    "report"
  );
  ...
  return (
    <Wrapper>
      <HeadCon>
        <BaseBreadcrumb
          data={[
            { name: "分析报告", link: "/qcThree/summaryReport" },
            { name: "报告详情", link: "" }
          ]}
        />
        ...
        <div className="aside">
          <span>
            由{report.creatorName}创建
            {report.updateTime && <span>，最后修改于{report.updateTime}</span>}
          </span>
        </div>
      ...
      <ScrollCon>
        <Page ref={pageRef} className="print-page">
          {qualityAnalysisReportViewModal.sectionList.map((item, index) => {
            if (item.sectionId) {
              let Components = qualityAnalysisReportViewModal.getSection(
                item.sectionId
              );
        ...
        {qualityAnalysisReportViewModal.baseModal && (
          <qualityAnalysisReportViewModal.baseModal.Component />
        )}
      </ScrollCon>
    </Wrapper>
  );
