...
import verifyGzsrm from './verify'
...
export default observer((props: Props) => {
  ...
  function setFormItem(item: object) {
    const keys = ['SR0005021', 'SR0005019', 'SR0005017', 'SR0005015', 'SR0005013', 'SR0005011', 'SR0005029'];
    const sum = keys.reduce((acc: number, cur: string) => {
      const val = form[cur];
      const valNum = isNaN(+val) ? 0 : +val;
      return acc + valNum;
    }, 0);
    setForm({ ...form, ...item, 'SR0005022': sum });
  }
  // 全部科室
  const getDeptAll = () => {
  ...
  const onVerify = () => {
    if (JSON.stringify(form) === '{}') {
      ...
      newArr.forEach((item: { name: string; title: any; }) => {
        verifyGzsrm.forEach((items, index) => {
          if (item.name === items.name) {
            verifyGzsrm[index].value = item.title
          }
        })
      })
      let is = verifyGzsrm.every((item: { value: string }) => item?.value)
      return is
    }
  }
  const handleSubmit = async (status: boolean) => {
    if (onVerify()) {
      if (notFullMarks('SR0005011', form.SR0005011)) {
  ...
  useEffect(() => {
    ...
    getDeptAll()
    setForm({
      'SR0005001': userName, 'SR0005003': moment(new Date).format("YYYY-MM-DD HH:mm"),
      'SR0005011': 15, 'SR0005013': 10, 'SR0005015': 25, 'SR0005017': 20, 'SR0005019': 5, 'SR0005021': 10, 'SR0005029': 15, 'SR0005022': 100
    })
  }, [])
            ...
            <table className={!appStore.queryObj.id ? '' : hasSubmit() ? '' : 'disable'}>
              <tbody>
                <tr>
                  ...
                  <td>
                    <Input
                      value={form.SR0005001}
                      onChange={(e) =>
                        setFormItem({ 'SR0005001': e.target.value })
                      }
                    />
                  </td>
                  <td className='required'>查房班次:</td>
                  <td>
                    <Select className='select' value={form.SR0005002}
                      onChange={(val: any) =>
                        setFormItem({ 'SR0005002': val })
                      }
                    >
                  ...
                  <td>
                    <DatePicker
                      ...
                      allowClear={false}
                      value={moment(form.SR0005003)}
                      showTime={{ defaultValue: moment('00:00', 'HH:mm') }}
                      onChange={_date => {
                        setFormItem({ 'SR0005003': moment(_date).format("YYYY-MM-DD HH:mm") })
                      }}
                      onOk={_data => {
                        setFormItem({ 'SR0005003': moment(_data).format("YYYY-MM-DD HH:mm") })
                      }}
                    />
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    <Select className='select inpatientAreaSel'
                      ...
                      value={form.SR0005004} onChange={(val: string) => {
                        ...
                        setWardCode(obj)
                        setFormItem({ 'SR0005004': newarr[0].name })
                      }
                      }>
                  ...
                  <td>
                    <Input
                      value={form.SR0005005}
                      onChange={(e) =>
                        setFormItem({ 'SR0005005': e.target.value })
                      }
                    />
                  </td>
                  ...
                  <td>
                    <Input
                      value={form.SR0005006}
                      onChange={(e) =>
                        setFormItem({ 'SR0005006': e.target.value })
                      }
                    />
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    <Input
                      value={form.SR0005007}
                      onChange={(e) =>
                        setFormItem({ 'SR0005007': e.target.value })
                      }
                    />
                  </td>
                  ...
                  <td>
                    <Input
                      value={form.SR0005008}
                      onChange={(e) =>
                        setFormItem({ 'SR0005008': e.target.value })
                      }
                    />
                  </td>
                  ...
                  <td>
                    <Input
                      value={form.SR0005009}
                      onChange={(e) =>
                        setFormItem({ 'SR0005009': e.target.value })
                      }
                    />
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td>
                    <Input.TextArea
                      value={form.SR0005010}
                      rows={6}
                      onChange={(e) =>
                        setFormItem({ 'SR0005010': e.target.value })
                      }
                    />
                  </td>
                  <td>
                    <InputNumber value={form.SR0005011} min={0} max={15} step={0.1}
                      onChange={(value) => {
                        form.SR0005011 = value
                        setFormItem({ 'SR0005011': value })
                      }
                      }
                    />
                  </td>
                ...
                <tr>
                  ...
                  <td>
                    <Input.TextArea
                      value={form.SR0005012}
                      rows={6}
                      onChange={(e) =>
                        setFormItem({ 'SR0005012': e.target.value })
                      }
                    />
                  </td>
                  <td>
                    <InputNumber value={form.SR0005013} min={0} max={10} step={0.1}
                      onChange={(value) => {
                        form.SR0005013 = value
                        setFormItem({ 'SR0005013': value })
                      }
                      }
                    />
                  </td>
                ...
                <tr>
                  ...
                  <td>
                    <Input.TextArea
                      value={form.SR0005014}
                      rows={6}
                      onChange={(e) =>
                        setFormItem({ 'SR0005014': e.target.value })
                      }
                    />
                  </td>
                  <td>
                    <InputNumber value={form.SR0005015} min={0} max={25} step={0.1}
                      onChange={(value) => {
                        form.SR0005015 = value
                        setFormItem({ 'SR0005015': value })
                      }}
                    />
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    <Input.TextArea
                      value={form.SR0005016}
                      rows={8}
                      onChange={(e) =>
                        setFormItem({ 'SR0005016': e.target.value })
                      }
                    />
                  </td>
                  <td>
                    {/* <Input.TextArea
                      value={form.SR0005017}
                      rows={8}
                      onChange={(e) =>
                        setFormItem({ 'SR0005017': e.target.value })
                      }
                    /> */}
                    <InputNumber value={form.SR0005017} min={0} max={20} step={0.1}
                      onChange={(value) => {
                        form.SR0005017 = value
                        setFormItem({ 'SR0005017': value })
                      }
                      }
                    />
                  </td>
                ...
                <tr>
                  ...
                  <td>
                    <Input.TextArea
                      value={form.SR0005018}
                      rows={6}
                      onChange={(e) =>
                        setFormItem({ 'SR0005018': e.target.value })
                      }
                    />
                  </td>
                  <td>
                    <InputNumber value={form.SR0005019} min={0} max={5} step={0.1}
                      onChange={(value) => {
                        form.SR0005019 = value
                        setFormItem({ 'SR0005019': value })
                      }
                      }
                    />
                  </td>
                ...
                <tr>
                  ...
                  <td>
                    <Input.TextArea
                      value={form.SR0005020}
                      rows={6}
                      onChange={(e) =>
                        setFormItem({ 'SR0005020': e.target.value })
                      }
                    />
                  </td>
                  <td>
                    <InputNumber value={form.SR0005021} min={0} max={10} step={0.1}
                      onChange={(value) => {
                        form.SR0005021 = value
                        setFormItem({ 'SR0005021': value })
                      }
                      }
                    />
                  </td>
                ...
                <tr>
                  ...
                  <td>
                    <Input.TextArea
                      value={form.SR0005028}
                      rows={6}
                      onChange={(e) =>
                        setFormItem({ 'SR0005028': e.target.value })
                      }
                    />
                  </td>
                  <td>
                    <InputNumber value={form.SR0005029} min={0} max={15} step={0.1}
                      onChange={(value) => {
                        form.SR0005029 = value
                        setFormItem({ 'SR0005029': value })
                      }
                      }
                    />
                  </td>
              ...
              <tr>
                ...
                <tr className={allowRectification ? '' : 'disable'}>
                  ...
                  <td colSpan={5}>
                    <Input.TextArea
                      value={form.SR0005024}
                      rows={6}
                      onChange={(e) =>
                        setFormItem({ 'SR0005024': e.target.value })
                      }
                    />
                  </td>
                </tr>
                ...
                {/*  <td colSpan={5}>*/}
                {/*    <Input.TextArea*/}
                {/*      value={form.SR0005023}*/}
                {/*      rows={6}*/}
                {/*      onChange={(e) =>*/}
                {/*        setFormItem({ 'SR0005023': e.target.value })*/}
                {/*      }*/}
                {/*    />*/}
                {/*  </td>*/}
                {/*</tr>*/}
