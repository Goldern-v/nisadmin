...
export default observer(function (props) {
  ...
  const setImg = () => {
    let imgEl: any = document.querySelectorAll('.chart-img')
    if (imgEl.length) {
      for (let i = 0; i < imgEl.length; i++) {
        instance.imgList[i] && (imgEl[i].src = instance.imgList[i])
      }
    }
  }
  const onPrint = () => {
    instance.isPrint = true
  };
  useEffect(() => {
    if (instance.isPrint) {
      setImg()
  ...
  return (
    <Wrapper>
      <Spin spinning={instance.loading}>
        <HeadCon>
          {/* check: 需要修改 */}
          <BaseBreadcrumb data={[{ name: '质控报告', link: routePath() }, { name: '汇总详情', link: '' }]} />
          ...
          <div className='tool-con'>
            ...
            {queryObj.id && report?.status == '0' && <Button loading={instance.loading} onClick={handleDel}>删除</Button>}
