...
export default function CreateAnalysisModal(props: Props) {
    ...
    if (current) {
      ...
      current
        ...
        .then((res: any) => {
          ...
          if (templateName == type1.value) {
            params.reportMonth = reportMonth
          } else if (templateName == type2.value) {
  ...
  const setReportName = () => {
    ...
    if (current) {
      ...
      const date = type1.value === templateName ? reportMonth + '月' : (quarterList[Number(reportQuarter) - 1] + (flag ? '（含月份）' : ''))
      let reportName = `${year}年${date}护理质量汇总`
