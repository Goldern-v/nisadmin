...
import getEditorControls from 'src/modules/badEvents/views/editor/configs/controls'
import LinkEditor from 'src/modules/badEvents/views/editor/components/business/LinkEditor'
import HeadingPicker from 'src/modules/badEvents/views/editor/components/business/Headings'
import TextColorPicker from 'src/modules/badEvents/views/editor/components/business/TextColor'
import FontSizePicker from 'src/modules/badEvents/views/editor/components/business/FontSize'
import LineHeightPicker from 'src/modules/badEvents/views/editor/components/business/LineHeight'
import FontFamilyPicker from 'src/modules/badEvents/views/editor/components/business/FontFamily'
import TextAlign from 'src/modules/badEvents/views/editor/components/business/TextAlign'
import EmojiPicker from 'src/modules/badEvents/views/editor/components/business/EmojiPicker'
import LetterSpacingPicker from 'src/modules/badEvents/views/editor/components/business/LetterSpacing'
import TextIndent from 'src/modules/badEvents/views/editor/components/business/TextIndent'
import DropDown from 'src/modules/badEvents/views/editor/components/common/DropDown'
import { ContentUtils } from 'braft-utils'
import { showModal } from 'src/modules/badEvents/views/editor/components/common/Modal'
import { getExtensionControls } from 'src/modules/badEvents/views/editor/helpers/extension'
      ...
      <div className={`bf-controlbar ${className || ''}`} style={style} onMouseDown={this.preventDefault}>
        ...
        allControls.map((item, index) => {
          ...
          if (itemKey.toLowerCase() === 'separator') {
            return <span key={index} className='separator-line' />
          }
          ...
          if (controlItem.type === 'headings') {
            ...
            return (
              <FontSizePicker key={index} fontSizes={fontSizes} defaultCaption={controlItem.title} {...commonProps} />
            )
            ...
            return <TextIndent key={index} defaultCaption={controlItem.title} {...commonProps} />
            ...
            return <EmojiPicker key={index} emojis={emojis} defaultCaption={controlItem.text} {...commonProps} />
          } else if (controlItem.type === 'link') {
            return <LinkEditor key={index} defaultLinkTarget={defaultLinkTarget} {...commonProps} />
          } else if (controlItem.type === 'text-align') {
            return <TextAlign key={index} textAligns={textAligns} {...commonProps} />
                  ...
                  if (controlItem.modal && controlItem.modal.id) {
                    if (this.extendedModals[controlItem.modal.id]) {
                      ...
                      controlItem.modal.onCreate &&
                        controlItem.modal.onCreate(this.extendedModals[controlItem.modal.id])
                    }
                  }
            ...
            return (
              <div key={index} className={`component-wrapper ${controlItem.className || ''}`}>
                {controlItem.component}
              </div>
            )
