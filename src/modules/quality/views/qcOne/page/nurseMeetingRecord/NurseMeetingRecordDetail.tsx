...
export default observer(function NurseMeetingRecordDetail() {
  ...
  const getDetail = () => {
    setLoading(true);
    nurseMeetingRecordService.getDetail(search.id, wardCode).then(
      (res) => {
        if (res.data) {
          setLoading(false);
          setNurseMeeting({
  ...
  const handleDelete = (record: any) => {
    Modal.confirm({
      ...
      onOk: () => {
        setLoading(true);
        nurseMeetingRecordService.delete(record.id).then(
          (res) => {
  ...
  const handleRead = () => {
    setLoading(true);
    nurseMeetingRecordService.read(search.id).then(
      (res) => {
  ...
  const ReadBtn = () => {
    if (nurseMeeting.status !== "1") return <span />;
    let target = unreceiverList.find(
      (item: any) =>
        item.empNo.toLowerCase() ==
        (authStore.user && authStore.user.empNo.toLowerCase())
    );

    if (!target) return <span />;
    else
      return (
        <Button disabled={loading} type="primary" ghost onClick={handleRead}>
          我已查看
        </Button>
      );
  };
  ...
  const readingStatus = () => {
    let target = receiverList.find(
      (item: any) =>
        item.empNo.toLowerCase() ==
        (authStore.user && authStore.user.empNo.toLowerCase())
    );
    if (target) return <span>已读</span>;
  ...
  const handlePush = () => {
    Modal.confirm({
      ...
      onOk: () => {
        ...
        setLoading(true);
        nurseMeetingRecordService.saveOrUpdate(params).then(
          (res) => {
            setLoading(false);
      ...
      <TopPannel>
        <TopHeader>
          ...
          <div className="topHeaderTitle">
            <div className="title">{nurseMeeting.meetingDay} 会议记录详情</div>
            <div className="topHeaderButton">
              ...
              {editable() && nurseMeeting && nurseMeeting.status == "0" && (
                <Button
                  disabled={loading}
                  type="danger"
                  ghost
                  onClick={() => handleDelete(nurseMeeting)}
                >
                  删除
                </Button>
              )}
          ...
          <Spin spinning={loading}>
            <MessageBox>
              <div className="boxLeft">
                <div>
                  会议日期：{nurseMeeting.meetingDay} {nurseMeeting.meetingTime}
                </div>
                <div>科室：{nurseMeeting.wardName}</div>
                <div>
                  会议主持：
                  {comperes.map((item: any) => item.empName).join("、")}
                </div>
              </div>
              <div className="boxRight">
                <div>
                  会议种类：
                  {nurseMeeting.meetingType == "QCWMT001" ? "周会" : "月会"}
                </div>
                <div>会议地点：{nurseMeeting.meetingLocation}</div>
                <div>
                  记录人：
                  {recorders.map((item: any) => item.empName).join("、")}
                </div>
              </div>
              <div>
                到会人员：
                {attendees.map((item: any) => item.empName).join("、")}
              </div>
              <div className="boxLeft">
