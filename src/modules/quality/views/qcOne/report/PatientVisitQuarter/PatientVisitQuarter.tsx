...
export default observer(function PatientVisitQuarter() {
  ...
  const handleExportGather = (isBigDept?: boolean) => {
    let year = query.year
    let month = query.month || moment().quarter().toString()
    let $wardCode = ''
    ...
    <HeaderCon>
      ...
      <RightIcon>
        ...
        <Select
          ...
          style={{ width: '110px' }}>
          <Option value=''>全部</Option>
          {statusList.filter((item: any) => !item.disabled).map((item: any) => <Option value={`${item.code}`} key={item.code}>{item.name}</Option>)}
        </Select>
        <span>科室:</span>
