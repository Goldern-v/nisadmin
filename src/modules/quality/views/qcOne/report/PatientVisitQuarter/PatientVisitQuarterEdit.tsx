...
export default observer(function PatientVisitQuarterEdit() {
  ...
  useEffect(() => {
    ...
    patientVisitQuarterModel.init(query)
  }, [])

  let report: Report = patientVisitQuarterModel.getDataInAllData('report')

  let reportName = patientVisitQuarterModel.getSectionData('报告名称')!.text || report.reportName
  ...
  const onDelete = () => {
    globalModal.confirm('删除确认', '你确定要删除该报告吗？').then((res) => {
      patientVisitQuarterService.delete(qs.parse(appStore.query)).then((res) => {
  ...
  const onPublish = () => {
    globalModal.confirm('提交确认', '你确定要提交该报告吗？').then((res) => {
      patientVisitQuarterService.publish(qs.parse(appStore.query)).then((res) => {
  ...
  const onCancelPublish = () => {
    globalModal.confirm('撤销确认', '你确定要撤销该报告吗？').then((res) => {
      patientVisitQuarterService.cancelPublish(qs.parse(appStore.query)).then((res) => {
    ...
    <Wrapper>
      <HeadCon>
        ...
        <div className='tool-con'>
          ...
          {(isRoleManage || isDepartment || isSupervisorNurse) && <Button onClick={() => onPrint(true)}>打印</Button>}
          <Button onClick={() => appStore.history.goBack()}>返回</Button>
        </div>
      </HeadCon>
      <ScrollCon>
        <Page ref={pageRef} className='print-page'>
          {patientVisitQuarterModel.sectionList.map((item, index) => {
            if (item.sectionId) {
              let Components = patientVisitQuarterModel.getSection(item.sectionId)
        ...
        {patientVisitQuarterModel.baseModal && <patientVisitQuarterModel.baseModal.Component />}
      </ScrollCon>
    </Wrapper>
  )
})
