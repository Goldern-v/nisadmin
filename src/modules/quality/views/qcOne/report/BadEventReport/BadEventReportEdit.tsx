...
export default observer(function badEventReportEdit() {
  ...
  useEffect(() => {
    ...
    badEventReportEditModel.init(query)
  }, [])

  let report: Report = badEventReportEditModel.getDataInAllData('report')

  let reportName = badEventReportEditModel.getSectionData('报告名称')!.text || report.reportName
    ...
    <Wrapper>
      ...
      <ScrollCon>
        <Page ref={pageRef} className='print-page'>
          {badEventReportEditModel.sectionList.map((item, index) => {
            if (item.sectionId) {
              let Components = badEventReportEditModel.getSection(item.sectionId)
        ...
        {badEventReportEditModel.baseModal && <badEventReportEditModel.baseModal.Component />}
      </ScrollCon>
    </Wrapper>
  )
})
