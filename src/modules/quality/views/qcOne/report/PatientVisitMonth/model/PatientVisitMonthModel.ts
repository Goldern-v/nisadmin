...
class PatientVisitMonthModel {
  ...
  async initData(query?: any) {
    let { data } = await patientVisitMonthService.getReport(query)
    this.allData = data
    /** 本月 */
    ...
    // this.getSectionData('月度随访')!.list = this.allData.patientVisitList || [];
...
export const patientVisitMonthModel = new PatientVisitMonthModel()
