...
export default observer(function StarRatingReportEdit() {
  ...
  const onDelete = () => {
    globalModal.confirm('删除确认', '你确定要删除该报告吗？').then((res) => {
      starRatingYearReportService.delete(qs.parse(appStore.query)).then((res) => {
        message.success('删除成功')
        setTimeout(() => {
          appStore.history.push('/qcOne/starRatingYearReport')
        }, 500)
      })
    })
  }
  const onPublish = () => {
    globalModal.confirm('提交确认', '你确定要提交该报告吗？').then((res) => {
      starRatingYearReportService.publish(qs.parse(appStore.query)).then((res) => {
  ...
  const onCancelPublish = () => {
    globalModal.confirm('撤销确认', '你确定要撤销该报告吗？').then((res) => {
      starRatingYearReportService.cancelPublish(qs.parse(appStore.query)).then((res) => {
  ...
  return (
    <Wrapper>
      <HeadCon>
        <BaseBreadcrumb
          data={[{ name: '星级考核报表', link: '/qcOne/starRatingYearReport' }, { name: '报告详情', link: '' }]}
        />
