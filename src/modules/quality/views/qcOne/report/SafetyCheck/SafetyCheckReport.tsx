...
import qs from 'qs'
import { safetyCheckReportService, ListQuery } from './api/SafetyCheckReportService'
...
export default observer(function SafetyCheckReport() {
  ...
  const columns: ColumnProps<any>[] = [
    ...
    },
    {
      ...
      align: 'center',
      width: 110,
      render: (text: string, record: any, idx: number) => `${record.year}年第${numToChinese(record.month)}季度`
    },
    {
      dataIndex: 'status',
      title: '状态',
    ...
    },
    {
      ...
      render: (text: string, record: any) => {
        return <DoCon className="operate-group">
          ...
          {(isRoleManage || isSupervisorNurse || isDepartment) && <span onClick={() => handleExport(record)}>导出</span>}
        </DoCon>
      }
    }
  ]
  ...
  const getList = (query: any) => {
    setLoading(true)
    safetyCheckReportService.getPage(query).then(res => {
      setLoading(false)
      if (res.data) {
  ...
  const handleEdit = (record: any) => {
    history.push(`/safetyCheckEdit?${qs.stringify({
  ...
  const handlePublish = (record: any) => {
    globalModal.confirm('提交确认', '你确定要提交该报告吗？').then((res) => {
      setLoading(true)
      safetyCheckReportService.publish({
        wardCode: record.wardCode,
        year: record.year,
        month: record.month
      }).then((res) => {
  ...
  const handleCancelPublish = (record: any) => {
    globalModal.confirm('撤销确认', '你确定要撤销该报告吗？').then((res) => {
      setLoading(true)
      safetyCheckReportService.cancelPublish({
        wardCode: record.wardCode,
        year: record.year,
        month: record.month
      }).then((res) => {
  ...
  return <Wrapper>
    <HeaderCon>
      ...
      <RightIcon>
        ...
        {(isRoleManage && !isSupervisorNurse && !isDepartment) && <Button type="primary" onClick={handleCreate}>新建</Button>}
