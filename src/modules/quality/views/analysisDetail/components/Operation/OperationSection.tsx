...
export default observer(function OperationSection(props: Props) {
  ...
  return (
    <Wrapper>
      ...
      <div className='context context-title'>(3) (<div>{pageInfo && Number(pageInfo.reportMonth) - 1 != 0 ? Number(pageInfo.reportMonth) - 1 : 12}</div>月)科室DRGS情况：RW ＞2:<div>{value.rw1}</div>;RW ＜0.5:<div>{value.rw2}</div>;CMI值:<div>{value.cmi}</div>;低风险死亡率:<div>{value.lowRiskMortality}</div>。 </div>
