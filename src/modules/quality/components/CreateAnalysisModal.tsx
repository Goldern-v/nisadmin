...
export default function CreateAnalysisModal(props: Props) {
  ...
  const { visible, onCancel, onOk, groupRoleList, allowClear, loading, reportFn } = props
  ...
  const setReportName = () => {
    let current = refForm.current
    if (current) {
      let { year, indexInType } = current.getFields()
      if (!year || !indexInType) return
      ...
      let reportName = reportFn ? reportFn({year: yearStr, month: monthStr}) : `${yearStr}年${monthStr}月${groupRoleName}反馈与分析`
