...
import { wrongQuestionBankModel_hj1 } from '../model/WrongQuestionBankModel'
...
export default observer(function WrongQuestionBank() {
  let { searchingContent } = wrongQuestionBankModel_hj1.query;
  ...
  useEffect(() => {
    ...
    wrongQuestionBankModel_hj1.setStatus(search.status || '待处理');
    wrongQuestionBankModel_hj1.setQuery(newQuery);
    wrongQuestionBankModel_hj1.getList();
  }, []);

  // useEffect(() => {
  //   if (wrongQuestionBankModel_hj1.tableData.length == 0 && wrongQuestionBankModel_hj1.tableLoading) return
  //   questionBankManageService.getCountMenu().then(res => {
  //     setMenuNum(res.data)
  //   })
  // }, [wrongQuestionBankModel_hj1.tableData])
  ...
  const TAB_CONFIG = [
    {
      ...
      orginTitle: '待处理',
      size: '',
      component: <WrongQuestionTable model={wrongQuestionBankModel_hj1} surplusHeight={surplusHeight} />
    },
    {
      title: '已处理',
      orginTitle: '已处理',
  ...
  const onTabsChange = (activeKey: string) => {
    ...
    let newQuery = {
      ...wrongQuestionBankModel_hj1.query,
      status: status || '待处理',
      pageIndex: 1
    }

    wrongQuestionBankModel_hj1.setQuery(newQuery);
    wrongQuestionBankModel_hj1.setStatus(status)
    wrongQuestionBankModel_hj1.getList();
    //更新url
    let url = match.url;
