...
export default function NewNursingRulesAddModal(props: Props) {
  ...
  const handleOkBtn = () => {
    if (refForm && refForm.current) {
      ...
      if (!formData.institutionName) return Message.error('未填写制度名称')

      if (!formData.fileType) return Message.error('未选择制度类型')

      if (!formData.catalog) return Message.error('未选择所属目录')
      ...
      let publicUse = 0
      if (!formData.deptCode || formData.deptCode == '公共' || formData.deptCode == '000000') publicUse = 1
  ...
  const FileContent = () => {
    if (visible) {
      return <Input type='file' className='file-content' accept={uploadAccept} onChange={handleImportFileChange} />
    } else return ''
  }
            ...
            <span className='content'>
              <div className='ipt'>
                <Form.Field name='deptCode'>
                  <Select value={deptCode} onSelect={handleSelect}>
