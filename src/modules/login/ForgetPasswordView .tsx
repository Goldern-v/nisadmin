...
import { message, Input, Tooltip,InputNumber } from "antd";
...
export default withRouter(function ForgetPasswordView (props: Props) {
  ...
  const resetPassword = async () => {
    ...
    if (!(isNewPswd && isRePswd) || !value.empNo || !isPhone) return
    setLoginLoading(true)
    let updatePassword;
      updatePassword = service.authApiService.updatePasswordByPhoneNumber(value);
  ...
  const changePassword = (type: any) => {
    ...
    let phone = refForm.current.getField('phone')
    // if (!reg?.flag) return true
    const patt = eval(`/${reg?.rule}/`)
    ...
    if (type == 'phone') {
      setIsPhone(phoneNumberValidator(phone))
    }
  }

  const phoneNumberValidator = (value:any) => {
        ...
        >
          ...
          <Form ref={refForm} rules={rules} onChange={changePassword}>
            <Form.Field
              ...
              name="empNo"
            >
              <Input placeholder="请输入工号" className={isEmpNo ? 'error' : ''} />
            </Form.Field>
            {!isPhone ? <div style={{ 'position': 'relative' , 'color':'red'}}>请正确输入手机号码</div> :
              ""
            }
            <Form.Field
              label="手机号码:"
              name="phone"
            >
              <Input placeholder="请输入手机号码" className={!isPhone ? 'error' : ''} />
            </Form.Field>
            ...
            <div style={{ 'position': 'relative' }}>
              <Form.Field
                ...
                name="newPswd"
              >
                <Input.Password placeholder="请输入新密码" className={isErrorNew ? 'error' : ''} />
              </Form.Field>
            ...
            <div style={{ 'position': 'relative' }}>
              <Form.Field
                ...
                name="rePswd"
              >
                <Input.Password placeholder="请输入确认密码" className={isErrorRe ? 'error' : ''} />
              </Form.Field>
              <Tooltip title={reg?.ruleMsg}>
