...
export default observer((props: Props) => {
  ...
  return (
    <Wrapper>
      ...
      <Row>
        <Col span={8} className='title'>PICC置管位置：</Col>
        <Col span={16}>
          <Select
            className='full'
            value={itemDataMap.R0009002}
            onChange={(value: string) => setItemDataMap('R0009002', value)}>
            {config.R0009002.map((item, index) =>
              <Select.Option value={item.code} key={item.code}>{item.name}</Select.Option>
            )}
          </Select>
        </Col>
      ...
      <Row>
        <Col span={8} className='title'>PICC置管方式：</Col>
        <Col span={16}>
          <Select
            className='full'
      ...
      <Row>
        <Col span={8} className='title'>发生CLABS时PICC留置时长：</Col>
        <Col span={16}>
          <Input value={itemDataMap.R0009005} suffix="天"
                 onChange={(event => setItemDataMap('R0009005', event.target.value))}/>
        </Col>
      </Row>
    </Wrapper>
  )
