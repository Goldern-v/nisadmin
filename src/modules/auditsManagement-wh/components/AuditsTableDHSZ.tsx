...
export default observer(function AuditsTableDHSZ(props: Props) {
    ...
    const specialistModal = createModal(SpecialistModal);
    ...
    const toDetails = (row: any) => {
        ...
        switch (showType) {
            ...
            case 'nurseFile':
                return service.commonApiService
            ...
            case 'csr':
                return appStore.history.push(`/CommunityDetailsView/${row.othersMessage.id}`);
            case "badEventMaster":
                return window.open(
                    `/crNursing/manage/#${
            ...
            case "nursePromotion":
                ...
                return appStore.history.push(`/PromotionAduit?${qs.stringify(row)}`);
            case "nurseSecond":
                return aMServices.getDetail(row.othersMessage.id).then((res) => {
            ...
            case "leaveApplication":
              return appStore.history.push(`/selfNurseFile/leaveRecordDetail?id=${row.othersMessage.id}`);
            /**临邑--专科准入类型审核**/
