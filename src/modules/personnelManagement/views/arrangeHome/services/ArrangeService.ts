...
export default class ArrangeService extends BaseApiService {
  ...
  public saveOrUpdate(status: "0" | "1" | undefined, urlName: string) {
    let obj:any = {
      ...
      setting: appStore.hisMatch({
        ...
        other: sheetViewModal.sheetTableData.sort((a: any, b: any) => a.sortValue - b.sortValue).map(
          (item: any, index: number) => ({
            ...item,
            status,
            sortValue: index + 1,
  ...
  public schShiftSettingSaveOrUpdate(obj: any) {
    ...
    if (appStore.HOSPITAL_ID === 'zjhj')  return this.post(`schShiftSettingZjhj/save`, obj);
