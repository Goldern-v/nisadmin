...
export default observer(function DeptBorrow(props: Props) {
  ...
  const rules: Rules = {
    ...
    manyNameTransferFrom: (val) => {
      let num = formRef.current?.getField('numTransferFrom')
      if (num) num = parseInt(num, 10)
      if (isNaN(num) || num <= 0) return true

      if (val.length > num) return '超出借用人数'
    ...
    if (visible) {
      setTimeout(() => {
        ...
        if (Object.keys(editParams).length > 0) {
          let { deptNameTransferTo, numTransferFrom, detailTransferFrom, daysTransferFrom, endDate, startDate, manyNameTransferFrom } = editParams;

          manyNameTransferFrom =
            manyNameTransferFrom ?
              manyNameTransferFrom.split(',').filter((name: string) => name) :
              []
          ...
          let target = deptList.find((item: any) => item.name == deptNameTransferTo)
          if (target) {
            getDeptEmpList(target.code)
          } else {
            setDeptEmpList([])
          }
        } else {
  ...
  const checkForm = () => {
    ...
    if (current) {
      current.validateFields().then(res => {
        // console.log(res);
        if (current) {
          let { startDate, endDate, deptNameTransferTo, detailTransferFrom, daysTransferFrom, numTransferFrom, manyNameTransferFrom } = current.getFields();
          ...
          if (manyNameTransferFrom && manyNameTransferFrom.length > 0) {
            manyNameTransferFrom = manyNameTransferFrom.join(',')
          } else {
            manyNameTransferFrom = ''
          }
  ...
  const getDeptEmpList = (deptCode: string) => {
    api.getDeptEmpList(deptCode).then((res) => {
  ...
  return <Modal
    ...
    <Wrapper>
      <Form
        ...
        <Row>
          <Col span={5} className="label">借用人员：</Col>
          <Col span={19}>
            <Form.Field name="manyNameTransferFrom">
              <Select mode="multiple">
                {deptEmpList.map((item: any) => {
                  return (
                    <Option value={item.empName} key={item.empNo}>
