...
export default function EditEffectiveTimeModal(props: Props) {
  ...
  const onSave = async () => {
    ...
    let data = {
      ...
      startDate: `${moment(value.startDate_1).format('YYYY-MM-DD')} ${moment(value.startDate_2).format('HH:mm')}`,
      endDate: `${moment(value.endDate_1).format('YYYY-MM-DD')} ${moment(value.endDate_2).format('HH:mm')}`
    }
  ...
  const onFormChange = (name: string, value: any, form: Form<any>) => {
    ...
    if (time_1 && time_2 && (name == 'startDate_2' || name == 'endDate_2')) {
      let diff = Math.max((time_2.valueOf() - time_1.valueOf()) / 1000 / 60 / 60, 0).toFixed(2)
  ...
  useLayoutEffect(() => {
    ...
    if (refForm.current && visible) {
      ...
      refForm!.current!.setFields({
        ...
        startDate_2:
          data.schAddOrSubs && data.schAddOrSubs[0] && data.schAddOrSubs[0]!.startDate
            ? moment(data.schAddOrSubs[0]!.startDate)
            : null,
        endDate_1: moment(data.workDate),
        endDate_2:
          data.schAddOrSubs && data.schAddOrSubs[0] && data.schAddOrSubs[0]!.endDate
            ? moment(data.schAddOrSubs[0]!.endDate)
            : null,
        statusType:
          data.schAddOrSubs && data.schAddOrSubs[0] && data.schAddOrSubs[0]!.statusType
