...
export class Table extends React.Component {
  ...
  handleMouseUp = (event) => {
    ...
    if (this.state.colResizing) {
      ...
      nextColToolHandlers[this.__colResizeIndex - 1].width =
        (nextColToolHandlers[this.__colResizeIndex - 1].width || defaultColWidth) + colResizeOffset
      nextColToolHandlers[this.__colResizeIndex].width =
        (nextColToolHandlers[this.__colResizeIndex].width || defaultColWidth) - colResizeOffset
  ...
  handleCellMouseEnter = (event) => {
    if (this.__dragSelecting) {
      ...
      if (
        this.__dragSelectingEndColumnIndex !== this.__dragSelectingStartColumnIndex ||
    ...
    this.setState(
      ...
      () => {
        this.props.editor.setValue(
          TableUtils.insertColumn(this.props.editorState, this.tableKey, this.state.tableRows.length, columnIndex)
        )
      }
    )
  }
    ...
    if (selectedColumnIndex >= 0) {
      this.setState(
        ...
        () => {
          ...
          setImmediate(() => {
            this.props.editor.setValue(
              TableUtils.removeColumn(this.props.editorState, this.tableKey, selectedColumnIndex)
            )
          })
        }
      )
  ...
  insertRow = (event) => {
    ...
    this.setState(
      ...
      () => {
        this.props.editor.setValue(
          TableUtils.insertRow(this.props.editorState, this.tableKey, this.colLength, rowIndex)
        )
      }
    )
  }
        ...
        {colToolHandlers.map((item, index) => (
          ...
          >
            ...
            <div className='bf-col-tool-center'>
              <div data-index={index} data-role='remove-col' className='bf-remove-col' onClick={this.removeColumn}>
                <i className='bfi-bin' />
              </div>
            </div>
        ...
        {rowToolHandlers.map((item, index) => (
          ...
          >
            <div className='bf-row-tool-up'>
              <div data-index={index} data-role='insert-row' className='bf-insert-row-before' onClick={this.insertRow}>
                <i className='bfi-add' />
              </div>
            </div>
            <div className='bf-row-tool-center'>
              <div data-index={index} data-role='remove-row' className='bf-remove-row' onClick={this.removeRow}>
                <i className='bfi-bin' />
              </div>
            </div>
  ...
  createContextMenu() {
    ...
    return (
      ...
      >
        <div className='context-menu-item' onMouseDown={this.mergeCells} data-disabled={!cellsMergeable}>
          {this.language.mergeCells}
        </div>
        <div className='context-menu-item' onMouseDown={this.splitCell} data-disabled={!cellSplittable}>
