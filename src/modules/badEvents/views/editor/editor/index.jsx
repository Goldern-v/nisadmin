...
const isControlEnabled = (props, controlName) => {
  return (
    [...props.controls, ...props.extendControls].find((item) => item === controlName || item.key === controlName) &&
...
export default class BraftEditor extends React.Component {
  ...
  forceRender = () => {
    return this.setValue(
      ContentUtils.createEditorState(this.state.editorState.getCurrentContent(), this.editorDecorators)
    )
  }
  ...
  render() {
    ...
    language =
      (typeof language === 'function' ? language(languages, 'braft-editor') : languages[language]) ||
      languages[defaultProps.language]
    ...
    return (
      <div
        ...
        className={`bf-container ${className}${disabled ? ' disabled' : ''}${readOnly ? ' read-only' : ''}${
