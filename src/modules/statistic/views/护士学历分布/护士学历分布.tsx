...
export default observer(function 护士学历分布() {
  ...
  const getData = () => {
    setLoading(true)
    let jmfyQuery ={...query}
    statisticsApi.countEducation(appStore.HOSPITAL_ID !== 'jmfy'? {deptCode:query.deptCode}:jmfyQuery)
      .then((res: any) => {
        setLoading(false)
    ...
    body={<Spin spinning={loading}>
      ...
      {chartVisible && <ChartCon style={{ height: `${chartHeight || 0}px` }}>
        ...
        <div ref={tablePrintRef} style={{display:'none'}}>
          <PrintTable dataSource={data} columns={columns} title={'护士学历分布'}></PrintTable>
        </div>
