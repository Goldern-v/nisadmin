...
import QHWYShiftChoose from './components/NurseByShiftChoose_qhwy'
    ...
    const getShiftAndRange = (callback?: Function) => {
        Promise.all([
            ...
            .then(res => {
                ...
                newFilterObj['shift_type'].list = shiftTypeList.map((name: string) => ({
                    name,
                    checked: shiftTypeSelected
                }))


                let rangeNameSelected = newFilterObj['range_name'].checked
                newFilterObj['range_name'].list = rangeNameList.map((name: string) => ({
                    name,
                    checked: rangeNameSelected
                }))
        ...
        <Con>
            ...
            <MidMidCon>
                <LeftCon>
                    ...
                    {/* 对应表 */}
                    <TableCon>
                        <TableFirst byHours={{morningHourTableData, nightHourTableData}} statusRadio={statusRadio}
                                    tableData={tableData} hourMap={hourMap} morningMap={morningMap} nightMap={nightMap}
                                    filterObj={filterObj}/>
                    </TableCon>
                </LeftCon>
                <RigthCon>
                    <div className='NurseByShiftChooseCon'>
                        ...
                        {!['gxjb', 'qhwy'].includes(appStore.HOSPITAL_ID) && <NurseByShiftChoose
