...
export default observer(function WardLogEdit(props: any) {
        ...
        switch (item.type) {
            ...
            case '8':
                if(appStore.HOSPITAL_ID ==='qhwy'){
                    let list: any = item.content ? JSON.parse(item.content).map((item: any) => item.empName).join(',') : item.content.split('')
                    return (
    ...
    const handleAddPersonEdit = (value: any, index: number) => {
        selectPeopleModal.show({
            checkedUserList: value ? JSON.parse(value) : [],
            onOkCallBack: (payload) => {
                let arr: any[] = []
                formatRecievers(payload, arr)
                let list = editList.map((item: any, i: number) => {
                    if (item.type == 8) {
                        item.content = JSON.stringify(arr.map((i: any) => {
                            return {
            ...
            <div className="editor-contain">
                <Spin spinning={loading}>
                    ...
                    <div className="sub-title">
                        {editList.filter((item: any) => item.type === '7').map((item: any, idx: number) => <div
                            key={idx}>{item.content}</div>)}
                    </div>
                    ...
                    <div className="edit-row">
                        ...
                        <div className="content">
                            <Input.TextArea autosize={{minRows: 3}} value={remark}
