...
function WardRegisterDefaultRouter() {
  ...
  const leftMenuConfig = [
    ...
    ...registerList.map((item: any) => ({
      ...
      component: { ...默认病区登记表记本 },
      hide: ['whyx', 'fsxt','lyyz','qhwy','wjgdszd'].includes(appStore.HOSPITAL_ID) ? false : !item.menu,
      icon: <ZDHZ />,
      payload: {
      ...
      custom: {
        textColor: ['whyx', 'fsxt','lyyz','qhwy','wjgdszd'].includes(appStore.HOSPITAL_ID) ? item.color : ''
      }
    }))
  ]
  // 1：日 2：周 3：月 4：班次）
  ...
  const initNavList = () => {
    setLoading(true)
    if (['wjgdszd','whyx', 'fsxt','lyyz','qhwy'].includes(appStore.HOSPITAL_ID)) {
