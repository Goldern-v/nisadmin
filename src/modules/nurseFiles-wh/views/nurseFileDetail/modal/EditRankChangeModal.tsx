...
export default function EditRankChangeModal(props: Props) {
  ...
  const onSave = async () => {
    ...
    nurseFilesService.commonSaveOrUpdate('nurseWHHierarchy', { ...obj, ...value }).then((res: any) => {
  ...
  useLayoutEffect(() => {
    ...
    if (data && refForm.current && visible) {
      refForm!.current!.setFields({
        ...
        nursehierarchy: data.nursehierarchy,
    ...
    <Modal title={title} visible={visible} onOk={onSave} onCancel={onCancel} okText='保存' forceRender centered>
      <Form ref={refForm} rules={rules} labelWidth={120} onChange={onFieldChange}>
        <Row>
          ...
          <Col span={24}>
            <Form.Field label={`现层级`} name='nursehierarchy'>
              <Input />
            </Form.Field>
          </Col>
          <Col span={24}>
