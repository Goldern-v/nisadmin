...
export default function ChartCon(props: Props) {
  // let { list } = props
  const list: any = []
  let tslList = list.filter((item: any) => item.itemType == '新增患者量')
  let ydlList = list.filter((item: any) => item.itemType == '阅读患者量')
  let tslObj: any = { name: '新增患者量' }
  ...
  dv.transform({
    type: 'fold',
    fields: tslList.map((item: any) => item.time),
    key: '月份',
    value: '数量'
  })
  const scale = [
