...
import { ReactComponent as AgreeIcon } from 'src/global/images/morengouxuan.svg'
...
import { Tag } from 'src/vendors/antd'
...
export default function GroupsHandBookAduitModal(props: Props) {
  ...
  const onOk = () => {
    ...
    let obj = {
      saveAudites: (selectedRows || []).map((item: any) => ({
        commiterNo: item.commiterNo
      })),
      flagAudited: agree === 'agree' ? true : agree === 'disagree' ? false : '',
      detail: opinion
    }
    aMServices.batchAudited(obj).then((res) => {
    ...
    <Modal title='批量审核' visible={visible} onOk={onOk} onCancel={onCancel} okText='保存' forceRender width={800}>
      <MainPart>
        <AduitCon>
          <FormCon>
            <div className='row'>
              <div className='key'>审核人员：</div>
              <div className='value'>
                {selectedRows &&
                  selectedRows.map((item: any, index: number) => <Tag key={index}>{item.commiterName}</Tag>)}
              </div>
            </div>
            <div className='row'>
