    ...
    if (refForm && refForm.current) {
      ...
      current.validateFields().then(
        () => {
          ...
          flatManageProblemService.saveOrUpdate(data).then(
            (res) => {
        ...
        <Form ref={refForm} rules={rules} onChange={handleCFormChange}>
          ...
          <div className='row disabled'>
            ...
            <span className='content'>
              <Input
                className='ipt'
                value={params.inspectorName || (authStore.getUser() && authStore.getUser().empName)}
                disabled={true}
              />
                ...
                <Form.Field name='responsibleEmpNo'>
                  <Select className='ipt'>
                    {nurseList.map((item: any) => (
                      <Option value={item.empNo} key={item.empNo}>
                        {item.empName}
                      </Option>
                    ))}
                  </Select>
