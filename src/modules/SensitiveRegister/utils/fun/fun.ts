...
export function getFun(context: any) {
  ...
  const onInitData = async () => {
    // setPageLoading(true);
    await sensitiveRegisterService
  ...
  //   return await sensitiveRegisterService
  ...
  /** 行编辑禁用规则 */
  const cellDisabled = (record: any, item = {} as any ) => {
    ...
    if (registerCode === 'QCRG_22_3' && item.itemCode === '基础数据：时间') {
      return true
    }
  ...
  const handleAuditAll = (aside?: string, type?: 'audit' | 'sign') => {
    ...
    let req =
      sensitiveRegisterService.auditAll
        .bind(sensitiveRegisterService)
    if (_type == 'sign') req =
      sensitiveRegisterService.signAll
        .bind(sensitiveRegisterService)
