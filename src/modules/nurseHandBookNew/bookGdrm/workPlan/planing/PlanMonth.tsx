...
import {quarterList, monthList, weekList} from 'src/enums/date';
import {
    Badge, Spin,
    message,
    Select,
...
export default observer(function PlanMonth(props: Props) {
    ...
    const handleOk = () => {
        ...
        } else if (planDatas.pathname.indexOf('month') > -1 || (planDatas.pathname.indexOf('Week') > -1)) {
            paramter.month = planDatas.createMonth
        } else if (planDatas.pathname.indexOf('Week') > -1) {
            paramter.week = planDatas.createWeek
        }
        ...
        <Wrapper>
            ...
            <Spin spinning={planDatas.mainLoading}>
                <div className="main-contain">
                    <div className="left">
                        {planDatas.itemList.length < 1 &&
                            <Empty style={{marginTop: '200px', width: "100%"}} image={Empty.PRESENTED_IMAGE_SIMPLE}
                                   description={'创建一份' + planDatas.getEmptyTips(planDatas.pathname)}/>}
                        {
                    ...
                    <div className="content">
                        <Spin style={{marginTop: '20%', width: "50%"}} spinning={planDatas.detailLoading}
                              size="large"></Spin>
                        {/* 正在编辑就显示富文本编辑框 */}
                        {(planDatas.contentItem.status === 0 || planDatas.contentItem.status === -1) &&
                            <>
                                ...
                                {planDatas.contentItem.status === -1 &&
                                    <NodeList key={planDatas.contentItem.id || ''}/>}
                            </>
                        }
                        {planDatas.contentItem.status == null &&
                            <Empty style={{marginTop: '200px', width: "100%"}} image={Empty.PRESENTED_IMAGE_SIMPLE}
                                   description={'选择左侧' + planDatas.getEmptyTips(planDatas.pathname) + '或创建一份' + planDatas.getEmptyTips(planDatas.pathname)}/>}
                            ...
                            <div className='content-middle'>
                                <div className='btn-list'>
                                    ...
                                    <div className="btn-list-right">
                                        ...
                                        {planDatas.contentItem.status === 1 &&
                                            <Button className='mr-15' onClick={() => planDatas.cancelSubmit()}
                                                    type="danger">撤销</Button>}
                                        {/* <Button onClick={() => {alert('打印')}}>打印</Button> */}

                                    </div>
                                </div>
                                <div
                                    style={{padding: '0 10px', margin: '0 auto 20px', maxWidth: 'calc(100vw - 730px)'}}>
                                    <div className='content-title'>{planDatas.contentDetail.title}</div>
                                    <div className='html-content' id="health-content">
                                        <div className='' style={{maxWidth: 'calc(100vw - 730px)'}}
                                             dangerouslySetInnerHTML={{__html: planDatas.contentDetail.content || ''}}/>
                                    </div>
            ...
            >
                <div className='modal-content'>
                    ...
                    {(planDatas.pathname.indexOf('month') > -1 || planDatas.pathname.indexOf('Week') > -1) &&
                        <div className="item-row" style={{marginTop: '15px'}}>
                    ...
                    {planDatas.pathname.indexOf('Week') > -1 &&
                        <div className="item-row" style={{marginTop: '15px'}}>
                            <div className="label" style={{fontWeight: 'bold', marginBottom: '.3em'}}>周</div>
                            <Select className="mr-15"
                                    style={{width: '100%'}}
                                    value={planDatas.createWeek}
                                    onChange={(val: number) => {
                                        planDatas.createWeek = val
                                    }}>
                                {
                                    weekList.map((v: any, i: number) => (
