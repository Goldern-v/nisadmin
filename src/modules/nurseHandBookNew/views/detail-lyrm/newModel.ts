...
class NurseNewHandBookRecordModel {
    ...
    @observable public statusChange: number = Math.random()
    @observable public noticeObj: Notice = {} as Notice
    /**判断是走正常详情还是护理质量与安全监测指标详情**/
    @observable public isNotice: boolean = false
    @observable public nurseHandbookItemDtoList: any = []
    ...
    @action
    public setNoticeObj = (obj: Notice) => {
        this.noticeObj = obj
    }
    public handleSaveNotice = () => {
        if (Object.keys(this.noticeObj).length === 0) return
        nurseHandBookService.saveNotice(this.noticeObj).then(res => {
    ...
    public getDetail = async () => {
        this.loading = true;
        /** 情况检测用另外的接口 **/
        let apiFun: any = this.isNotice ? nurseHandBookService.getNurseById({id: this.id}) : nurseHandBookService.getNHRById({id: this.id})
        return apiFun.then((res: types.Obj) => {
            if (this.isNotice) {
                const {record: {detail, title = "", menuCode = ''}, nurseHandbookItemDtoList} = res.data;
                this.nurseHandbookItemDtoList = nurseHandbookItemDtoList
                ...
                const {record: {detail, title = "", menuCode = ''}} = res.data;
                this.detail = res.data;
                if (detail) {
            ...
            console.log("this.configFn)====", this.configFn);
...
export const nurseHandbookRecordModel = new NurseNewHandBookRecordModel();
