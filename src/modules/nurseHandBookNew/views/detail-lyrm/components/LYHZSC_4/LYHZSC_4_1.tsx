...
export default observer(function (props: Props) {
    ...
    // }
    const onInputValue = (e: any, key: string, i: number) => {
        let list: any = cloneDeep(model.nurseHandbookItemDtoList)
        model.nurseHandbookItemDtoList = list.map((item: any, keys: number) => {
                if (keys === i) {
                    item[key] = e
                    if (item.total && item.qualifiedItems) {
                        item['indicators'] = (item.qualifiedItems / item.total * 100).toFixed(2) + '%'
                    }
                }
            return item
            }
    ...
    return (
        <Wrapper className='con--a4' ref={model.ctxRef}>
            ...
            <table>
                ...
                {/*}*/}
                {
                    model.nurseHandbookItemDtoList.map((v: Obj, i: number) => {
                        return (
                            <tr>
                                <td>{i + 1}</td>
                                <td>{v.monitorContent}</td>
                                <td>{v.calculationMethod}</td>
                                <td>{v.qualified}</td>
                                <td><InputNumber className='cell-ipt ta-c' value={v.qualifiedItems}
                                                 onChange={(e: any) => onInputValue(e, 'qualifiedItems', i)}></InputNumber>
                                </td>
                                <td><InputNumber className='cell-ipt ta-c' value={v.total}
                                                 onChange={(e: any) => onInputValue(e, 'total', i)}></InputNumber></td>
                                <td>{v.indicators}</td>
                            </tr>
                        )
                    })
