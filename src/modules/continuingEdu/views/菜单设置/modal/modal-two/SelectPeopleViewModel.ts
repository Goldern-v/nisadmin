...
class SelectPeopleViewModel {
  ...
  @computed get currentTreeData() {
    if (this.stepState.length == 1) {
      if (this.stepState[0] == "默认科室") {
        return {
          ...
          list: ((this as any).selectTreeData[1].data.userList || []).map(
            (item: any) => ({
              ...item,
              userList: [item],
              label: item.empName,
        ...
        let { data, dataLabel } = this.selectTreeData.find(
          item => item.step == this.stepState[0]
        ) || {
          data: [],
          dataLabel: ""
        };
    ...
    if (this.stepState.length == 2) {
      ...
      let userData: any =
        data.find((item: any) => item[dataLabel || ""] == this.stepState[1]) ||
        {};
      return {
