...
function AuditInfoPannel(props: Props) {
      ...
      {(auditInfo || []).map((item: any, idx: number) =>
        <div className="audit-item" key={idx}>
          <div className="emp-img">
            {item.flag == 1 ? <img src={item.nearImageUrl} alt="" /> : <img src={authStore.user?.nearImageUrl} alt="" />}
          ...
          <div className="info">
            ...
            <div className="emp-name">{item.flag !== 1 ? authStore.user?.empName : item.handlerEmpName}</div>
...
export default AuditInfoPannel
