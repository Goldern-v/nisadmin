    ...
    <Modal title={trainExamData.modalTitle} visible={visible} 
      ...
      <Form>
        ...
        <Form.Item {...formItemLayout} label='考核年份'>
          {getFieldDecorator('year', {initialValue:moment(trainExamData.addExam.year) || moment(),
        ...
        <Form.Item {...formItemLayout} label='考核批次'>
          {getFieldDecorator('batch', {initialValue:trainExamData.addExam.batch || '',
        ...
        <Form.Item {...formItemLayout} label='考核日期'>
          {getFieldDecorator('examDate', {initialValue:moment(trainExamData.addExam.examDate) || moment(),
        ...
        // wrapperCol={{sm: { span: 20 },style:{float:'right'}}}
         label={'考核内容'}>
          {getFieldDecorator('examContent', {initialValue:trainExamData.addExam.examContent || '',
            rules: [{ required: true, message: '考核内容不能为空' }]
          })(<Input.TextArea autosize={{minRows: 3}} style={{ width: '100%' }} />)}
        </Form.Item>

        <Form.Item {...formItemLayout} label='主考科室'>
          {getFieldDecorator('deptItem', {initialValue:{key:trainExamData.addExam.examDeptCode,label:trainExamData.addExam.examDept},
        ...
        <Form.Item {...formItemLayout} label='合格成绩'>
          {getFieldDecorator('passScore', {initialValue:trainExamData.addExam.passScore || null,
            rules: [{ required: true, message: '合格成绩不能为空' }]
          })(<InputNumber step={1} min={1} style={{ width: '100%' }} />)}
        </Form.Item>

		<Form.Item {...formItemLayout} label='批注说明'>
          {getFieldDecorator('batchRemark', {initialValue:trainExamData.addExam.batchRemark || '',
