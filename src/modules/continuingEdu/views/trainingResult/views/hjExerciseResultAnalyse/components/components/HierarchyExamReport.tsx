...
export default observer(function HierarchyExamReport() {
  ...
  useEffect(() => {
    ...
    let timer = setTimeout(() => {
      let canvasEl =
        document.querySelector('.hj-exam-report-hierarchy1 canvas') as HTMLCanvasElement
      if (canvasEl) hjExerciseModal.hierarchyImgZhu = canvasEl.toDataURL()
      let canvasEl2 =
        document.querySelector('.hj-exam-report-hierarchy2 canvas') as HTMLCanvasElement
  ...
  const dv = new DataSet.View().source(hjExerciseModal.excelTableListByHierarchy);
  dv.transform({
    type: 'percent',
    field: 'personCount',
