...
class PromotionSettingModel {
  ...
  /** 专业训练重点 */
  @observable trainingKeyPointList = [] as any[]
  /** 岗位职责 */
  @observable jobDuty = ''
  ...
  @action public setJobDuty(newStr: string) {
    this.jobDuty = newStr
  }
  ...
  @action public getData() {
    ...
    if (target) {
      ...
      // 获取临床专业能力训练重点、岗位职责等信息
      this.trainingKeyPointList = []
      this.jobDuty = ''
      promotionSettingService
        .getPromoteConfigOther(target.title)
        .then(res => {
          if (res.data) {
            this.trainingKeyPointList = res.data.trainingKeyPoints || []
            this.jobDuty = res.data.jobDuty || ''
          }
        }, () => { })
    } else {
      this.loading = false
