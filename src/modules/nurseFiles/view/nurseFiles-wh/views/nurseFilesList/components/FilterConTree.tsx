...
export default observer(function FilterCon() {
  ...
  const getAllList = (list: any[], excludes: any[] = []): any[] => {
    return list.reduce((sum: any, cur: any) => {
      if (cur.children) {
        return [...sum, ...getAllList(cur.children as any[], excludes)]
      }
      if (!excludes.includes(cur.code)) {
        sum.push(cur.code)
      }
      return sum
    },
  ...
  const onFieldChange = async (name: string, text: any, form: Form<any>) => {
    ...
    if (value.deptCode && value.deptCode.length > 1) {
      let oldVal = statisticsViewModal.selectedDeptCode
      if (value.deptCode.includes("全院") && !oldVal.includes("全院")) {
        value.deptCode = ["全院"];
        form.setField("deptCode", value.deptCode);
        return;
      } else if (value.deptCode.includes("全院") && oldVal.includes("全院")) {
        value.deptCode = value.deptCode.filter((item: any) => item != "全院");
        form.setField("deptCode", value.deptCode);
        return;
      } else if (value.deptCode.includes("全部") && oldVal.includes("全部")) {
        value.deptCode = ["全部"];
        form.setField("deptCode", value.deptCode);
        return;
      } else if (value.deptCode.includes("全部") && oldVal.includes("全部")) {
    ...
    if (value.deptCode.length == 1 && value.deptCode[0] == "全部") {
      deptCodes = getAllList(statisticsViewModal.getDict("全部科室"), ['全部'])
    } else {
