...
export default function EditSocialNurseModal(props: Props) {
  ...
  const onSave = async (sign: boolean) => {
    ...
    nurseFilesService.commonSaveOrUpdate('nurseWHSocialJob', { ...obj, ...value, sign }).then((res: any) => {
    ...
    >
      <Form ref={refForm} rules={rules} labelWidth={120} onChange={onFieldChange}>
        <Row>
          <Row gutter={12}>
            <Col span={15}>
              <Form.Field label={`兼职时间`} name='startDate' required suffix='到'>
                <DatePicker />
              </Form.Field>
            </Col>
            <Col span={9}>
          ...
          <Col span={24}>
            <Form.Field label={`社会兼职名称`} name='socialJobName'>
              <Input />
            </Form.Field>
          </Col>
          {/* <Col span={24}>
            <Form.Field label={`兼职职位`} name='learnPosition'>
              <Input />
            </Form.Field>
          </Col> */}
          <Col span={24}>
            <Form.Field label={`兼职级别`} name='socialLevel'>
          ...
          <Col span={24}>
            <Form.Field label={`附件`} name='urlImageOne'>
              <MultipleImageUploader text='添加图片' tip={'上传兼职任职聘书扫描件'} />
            </Form.Field>
          </Col>
        </Row>
      </Form>
