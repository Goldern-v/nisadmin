...
import { Modal, Button, DatePicker, Select, Row, Col, message } from 'antd'
    ...
    >
      <Form ref={refForm} rules={rules} labelWidth={120} onChange={onFieldChange}>
        <Row>
          ...
          <Col span={24}>
            <Form.Field label={isSdlj ? '层级名称' : `现层级名称`} name='nursehierarchyNew'>
              <Select>
                {nurseFileDetailViewModal.getDict('层级').map((item: any, index: number) => (
                  <Option value={item.code} key={index}>
                    {item.name}
          ...
          <Col span={24}>
            <Form.Field label={isSdlj ? '开始时间' : `现层级开始时间`} name='startDate'>
              <DatePicker />
            </Form.Field>
          </Col>
         {isSdlj && <Col span={24}>
