...
export default function DeptChangeModal(props: Props) {
  ...
  useLayoutEffect(() => {
    ...
    if (info && refForm.current && visible) {
      let api = ['gxjb','qhwy','wjgdszd', 'whhk'].includes(appStore.HOSPITAL_ID) ? service.commonApiService.getNursingUnitAll() : service.commonApiService.deptInbigDeptListSelf()
      api.then((res) => {
        setDeptList(['gxjb','qhwy','wjgdszd', 'whhk'].includes(appStore.HOSPITAL_ID) ? res.data?.deptList : res.data)
      })
