...
function AddNursingModal(props: Props) {
  ...
  const onSave = (e: any) => {
    validateFields((err, value) => {
      ...
      if (value.deptCode)
        value.deptName = authStore.deptList.find(
          (item) => item.code == value.deptCode
        )!.name;
    ...
    >
      <Form>
        ...
        <Form.Item {...formItemLayout} label="是否护士">
          {getFieldDecorator("isNurse", {
            initialValue: true,
            rules: [{ required: true, message: "是否护士不能为空" }],
          })(
            <Radio.Group buttonStyle="solid">
            ...
            rules: [
              { required: true, message: "身份证号不能为空" },
              {
                validator(rule: any, value: any) {
                  ...
                  if (idCardNumberReg.test(value)) return Promise.resolve();
