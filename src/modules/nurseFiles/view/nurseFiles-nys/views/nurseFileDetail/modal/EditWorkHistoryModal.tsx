...
import { UNIT_LIST, PROFESSIONAL_WORK_LIST } from "../utils/index.enums";
// 加附件
...
export default function EditWorkHistoryModal(props: Props) {
  ...
  const onSave = async () => {
    ...
    nurseFilesService
      .nurseWorkExperienceAdd({ ...obj, ...value })
      .then((res: any) => {
        ...
        <Row>
          ...
          <Col span={24}>
            <Form.Field label={`专业技术工作`} name="professionalWork" required>
              <Select>
                {PROFESSIONAL_WORK_LIST.map((item: string) => (
                  <Select.Option value={item} key={item}>
                    {item}
                  </Select.Option>
                ))}
